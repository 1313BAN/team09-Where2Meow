...
public interface ReviewRepository extends JpaRepository<Review, Integer> {
     ...
     * 특정 여행지에 대한 리뷰 수 조회
     */
    @Query("SELECT COUNT(r) FROM Review r WHERE r.attraction.attractionId = :attractionId")
    Long countByAttractionId(@Param("attractionId") Integer attractionId);
    
    /**
     * 특정 여행지에 대한 평균 평점 조회
     */
    @Query("SELECT COALESCE(AVG(r.score), 0) FROM Review r WHERE r.attraction.attractionId = :attractionId")
